...
(不要实现，这个功能与运行时多态存在冲突！！！)
(unexpect无法与多态的实现共存，所以这里选择维持原有的多态)
(如果你有解决多态与unexpect兼容的办法的话，请你实施此)

如果一个结构仅需要某一个接口中的部分内容，那么你可以使用unexpect来禁用约束。
